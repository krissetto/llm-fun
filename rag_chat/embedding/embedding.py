...
# {chunk}
            ...
            raw_vectors = (
                await ai_client.embed(model=embeddings_model, input=f"search_document: {chunk}")
            ).get('embeddings')

            if semantic_vectors:
                embeddings.append((chunk, semantic_vectors))
            if raw_vectors:
                embeddings.append((chunk, raw_vectors))
...
async def generate_search_embeddings(
    ...
    if len(chat_thread) < 2:
        ...
        embeddings_res = await ai_client.embed(model=embeddings_model, input=f"search_query: {user_input}")
    else:
...
"""
        ...
        search_query_res = await ai_client.generate(
            model=llm_model,
        ...
        embeddings_res = await ai_client.embed(model=embeddings_model, input=f"search_query: {search_query_res.get('response')}")
        

    if not embeddings_res:
        print("Error creating embedding for the user's input")
        return []
    input_embeddings = embeddings_res.get("embeddings")
